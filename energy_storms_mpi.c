...
int main(int argc, char *argv[]) {
    ...
    //printf("the current processors is %d\n",rank);
    //fflush(stdout);
    ...
    /* 4. Storms simulation */
    for( i=0; i<num_storms; i++) {
        //printf("The current loop is %d and the current processors is %d\n",i,rank);
        //fflush(stdout);
        particle_per_proc = storms[i].size/(size-1);
        //printf("Particle_per_proc is  %d\n",particle_per_proc);
        if(particle_per_proc<1) {
            fprintf(stderr,"Error: The number of processors exceed the number of particles\n");
            printf("Please decrease the number of processors.\n");
            exit( EXIT_FAILURE );            
        }
        ...
        if (rank==0){
            ...
            for(int j = (rank-1)*particle_per_proc; j<storms[i].size;j++) {
                //printf("Current rank is %d, the processor size is %d\n", rank,size);
